...
const handleGetMethod = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const response = await axios.get(`${process.env.BACKEND_URL}/meal-likes`, {
        headers: {
...
const handlePostMethod = async (req: NextApiRequest, res: NextApiResponse) => { 
    const {accessToken} = await getAccessToken(req, res)
    const response = await axios.post(`${process.env.BACKEND_URL}/meal-likes/${req.body.id}`, req.body, {
        headers: {
            'Content-Type': 'application/json',
            'Access-Control-Allow-Origin': '*',
            'Access-Control-Allow-Methods': 'GET,PUT,POST,DELETE,PATCH,OPTIONS',
...
const handleDeleteMethod = async (req: NextApiRequest, res: NextApiResponse) => {
    const {accessToken} = await getAccessToken(req, res)
    const response = await axios.delete(`${process.env.BACKEND_URL}/meal-likes/${req.body.id}`, {
        headers: {
            'Content-Type': 'application/json',
            'Access-Control-Allow-Origin': '*',
            'Access-Control-Allow-Methods': 'GET,PUT,POST,DELETE,PATCH,OPTIONS'
