...
const handlePutMethod = async (req: NextApiRequest, res: NextApiResponse) => {
  const {accessToken} = await getAccessToken(req, res)
  const response = await axios.put(`${process.env.BACKEND_URL}/meals/${req.query.mealId}`,req.body, {
    headers: {
        'Content-Type': 'application/json',
        'Access-Control-Allow-Origin': '*',
        'Access-Control-Allow-Methods': 'GET,PUT,POST,DELETE,PATCH,OPTIONS',
...
const handleDeleteMethod = async (req: NextApiRequest, res: NextApiResponse) => {
  const {accessToken} = await getAccessToken(req, res)
  const response = await axios.delete(`${process.env.BACKEND_URL}/meals/${req.query.mealId}`, {
    headers: {
        'Content-Type': 'application/json',
        'Access-Control-Allow-Origin': '*',
        'Access-Control-Allow-Methods': 'GET,PUT,POST,DELETE,PATCH,OPTIONS',
